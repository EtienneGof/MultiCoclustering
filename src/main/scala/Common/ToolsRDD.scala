...
object ToolsRDD extends java.io.Serializable {
  ...
  def fromCellDistributionToRowDistribution(data : RDD[(Int, Int, DenseVector[Double])]): RDD[(Int, Array[DenseVector[Double]])] = {
  ...
  def dataWithProbFromDataWithPartitions(data: RDD[(Int, Int, DenseVector[Double], Int, Int)], K:Int, L:Int): RDD[(Int, Int, DenseVector[Double], DenseVector[Double], DenseVector[Double])] = {
    data.map(r =>
      (r._1, r._2, r._3,
        DenseVector(intToVecProb(r._4, K)),
        DenseVector(intToVecProb(r._5, L))   )
    )
  }

  def rowPartitionFromRDDPartition(data: RDD[(Int, Array[DenseVector[Double]], DenseVector[Double])]): List[Int] = {
  ...
  def getBiPartitionFromMAPDataCells(data: RDD[(Int, Int, DenseVector[Double], DenseVector[Double], DenseVector[Double])])(implicit ss: SparkSession): (List[Int], List[Int]) = {
    val rowPartition = data.map(r => (r._1, argmax(r._4.toArray))).distinct.collect().sortBy(_._1).map(_._2).toList
    val colPartition = data.map(r => (r._2, argmax(r._5.toArray))).distinct.collect().sortBy(_._1).map(_._2).toList
    (rowPartition, colPartition)
  }

  def getLBMDataPerRowsFromDataPerCells(data: RDD[(Int, Int, DenseVector[Double], Int, Int)]): RDD[(Int, Array[DenseVector[Double]], Int)] = {
    data.map(r =>
  ...
  def getCLBMDataPerRowsFromDataPerCells(data: RDD[(Int, Int, DenseVector[Double], Int, Int)]): RDD[(Int, Array[DenseVector[Double]], List[Int])] = {
  ...
  def getLBMDataPerColsFromDataPerCells(data: RDD[(Int, Int, DenseVector[Double], Int, Int)]): RDD[(Int, Array[DenseVector[Double]], Int)] = {
    data.map(r =>
